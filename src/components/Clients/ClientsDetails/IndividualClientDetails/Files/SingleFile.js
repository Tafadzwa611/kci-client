...
function SingleFile({fileObj, files, clientId, setFiles}) {
    ...
    return (
        <tr>
            ...
            <td style={{backgroundColor: fileObj.ok ? 'white': 'red', paddingLeft:"0"}}>{fileObj.name} <b>{fileObj.reason}</b></td> :
            <td style={{paddingLeft:"0"}}><a href={`/media/${fileObj.path}`} download>{fileObj.name}</a></td>}
