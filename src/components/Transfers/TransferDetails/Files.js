...
function Files({transfer, setTransfer}) {
  ...
  const uploadFile = (file, url, fileName, setProgress) => {
    return new Promise((res, rej) => {
      ...
      xhr.onload = () => {
        ...
        postXHR.open('POST', `/acc-api/transfer_file_upload/${transfer.id}/`);
        ...
        postXHR.onload = () => {
          ...
          setTransfer(curr => ({...curr, files: [...curr.files, jsonResponse]}));
  ...
  const deleteFile = async (evt) => {
    ...
    try {
      await axios.delete(`/acc-api/delete_transfer_file/${evt.target.id}/`, CONFIG);
      setTransfer(curr => ({...curr, files: curr.files.filter(file => file.id != evt.target.id)}));
    } catch(error) {
      console.log(error);
