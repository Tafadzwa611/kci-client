...
function UndoWriteOff({loanId, setOpen, setLoan, updateLoanList, setLoanData}) {
  const onSubmit = async (_, actions) => {
    try {
      const CONFIG = {headers: {'X-CSRFToken': Cookies.get('csrftoken'), 'Accept': 'application/json', 'Content-Type': 'application/json'}};
      const response = await axios.patch(`/loansapi/undo_write_off/${loanId}/`, {}, CONFIG);
          ...
          <Form>
            <NonFieldErrors errors={errors}>
              <ActionModalDialog 
                ...
                msg={'Are you sure you want to undo write off.'}
                setOpen={setOpen}
                act={'Undo Write Off'}
              />
            </NonFieldErrors>
