...
function SolidarityGroupForm({product}) {
  const initialValues = {
  ...
  const fetchMembers = async (groupId, setFieldValue) => {
    const response = await axios.get(`/clientsapi/group_members/${groupId}`);
    const members = response.data.map(member => ({...member, amount: 0}));
        ...
        <Form>
          <NonFieldErrors errors={errors}>
            ...
            <CustomInput
              ...
              type='number'
              onChange={(evt) => {
                const members = values.members.filter(member => {
                  if (member.id == evt.target.name) {
                    member.amount = evt.target.value;
                  }
                  return member;
                });
                setFieldValue('members', members);
