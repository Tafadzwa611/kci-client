...
import MiniTable from './MiniTable';
import MiniGroupDetails from './MiniGroupDetails';
import { Fetcher, Modal } from '../../../common';
import ReactHTMLTableToExcel from 'react-html-table-to-excel';
import Pager from './Pager';
  ...
  return (
    <>
      ...
      <div
        ...
        className={
          !isMobile && groupId
            ? 'table-container journal__table font-12'
            : 'table-container full__width font-12'
        }
      >
        <div
          className={
            !isMobile && groupId
              ? 'table-responsive journal__table-container-journals'
              : 'table-responsive full__table'
          }
        >
          {!isMobile && groupId ? (
            <MiniTable
          ...
          {!isMobile && groupId && (
            <Fetcher
              urls={[`/clientsapi/group/${groupId}/`]}
              extra={{ groupDetails, setGroupDetails, setGroupId, setGroupsData }}
            >
              {({ data, extra }) => <MiniGroupDetails groupData={data[0]} extra={extra} />}
            </Fetcher>
          )}
      ...
      {isMobile && mobileGroupId && (
        <Modal
          open={Boolean(mobileGroupId)}
          onClose={() => setMobileGroupId(null)}
          title='Group Details'
        >
          <Fetcher
            urls={[`/clientsapi/group/${mobileGroupId}/`]}
            extra={{
              groupDetails,
              setGroupDetails,
              setGroupId: setMobileGroupId,
              setGroupsData
            }}
          >
            {({ data, extra }) => <MiniGroupDetails groupData={data[0]} extra={extra} />}
...
const TableHeader = ({ groupData, params, setGroupsData }) => {
  return (
    <div className='table-header'>
      ...
      <div style={{ display: 'flex', columnGap: '10px', alignItems: 'center' }}>
        <div style={{ marginTop: '6px' }}>
          Page {groupData.number} of {groupData.num_of_pages}
        </div>
        <div>
          <ReactHTMLTableToExcel
            id='test-table-xls-button'
