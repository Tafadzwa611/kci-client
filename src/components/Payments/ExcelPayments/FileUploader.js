...
function FileUploader({setFileName}) {
  ...
  return (
    <div style={{display:"flex", columnGap:"2rem", alignItems:"center"}}>
      ...
      <div>
        <button type='button' className='btn btn-success' style={{display:"flex", columnGap:"5px", alignItems:"center"}} onClick={() => document.getElementById('selectedFile').click()}>
          <i className='fas fa-folder-open'/> 
          <span>Browse...</span>
        </button>
      </div>
      {file != null ?
      <div className='table-responsive p-0' style={{marginTop: '1rem', display:"flex", columnGap:"2rem", alignItems:"center"}}>
        {file.name}
