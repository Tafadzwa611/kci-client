...
function PenaltiesPaymentsChart({branches, currencies}) {
  ...
  const [penaltiesCollections, setPenaltiesCollections]= useState([])
  ...
  async function fetchData() {
    try {
      let url = `/loansapi/penalties_payments_chart/?currency_id=${currencyId}&page_num=${pageNum.current}&components=Penalties`;
      ...
      if (response.ok) {
        ...
        setPenaltiesCollections(report);
        return report
      }else {
      ...
      <div className="book-value-section" style={{marginTop:"1.5rem"}}>
        ...
        <div className="chart-section">
          <div className="chart-scroller header">
            <h1 className="dashboard-section-title">Penalties Collections</h1>
          </div>
...
export default PenaltiesPaymentsChart;