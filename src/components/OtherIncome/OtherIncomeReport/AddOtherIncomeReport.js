...
function AddOtherIncomeReport({open, setOpen, setOtherIncomeR, branches, currencies}) {
    const [limits, setLimits] = useState({upperLimit: ''});
    const [currencyId, setCurrencyId] = useState('');
    const [selectedBranchesIds, setSelectedBranchesIds] = useState([]);
    const [otherincomername, setOtherIncomeRName] = useState('All time other income');
    ...
    async function fetchExpensesReport() {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setOpen(false);
                setLoading(false);
                return setOtherIncomeR(curr => [...curr, {...data, currencyIso: currencies.filter(currency => currency.id == currencyId)[0].shortname, currencyId, selectedBranchesIds, limits}]);
            }else {
    ...
    const getUrl = () => {
        return `/otherincomeapi/otherincome_report/?currency_id=${currencyId}
            ...
            <div className='modal-dialog modal-lg' style={{maxWidth:"calc(100% - 3rem)"}}>
                <div className='modal-content'>
                    <div className='modal-header'>
                        <span style={{fontWeight:"600"}}>{otherincomername}</span>
                    ...
                    <ModalBody
                        ...
                        setOtherIncomeRName={setOtherIncomeRName}
                    />
...
const ModalBody = ({
    ...
    const handleLimits = (evt) => {
        ...
        setLimits(newLimits);
        const newOtherIncomeName = getOtherIncomeReportName(newLimits);
        setOtherIncomeRName(newOtherIncomeName);

        if (newLimits.upperLimit === '') {return}

        if (Number(newLimits.upperLimit) <= 0) {
        setOtherIncomeRName('Error');
        return setErrorMsg('Upper Limit cannot be zero or than Lower zero.');
        }
    }

    const getOtherIncomeReportName = (newLimits) => {
        if (newLimits.upperLimit == '') {
            return 'All time other income'
        }

        if (newLimits.upperLimit == 1) {
            return 'From yesterday other income'
        }
        
        if (newLimits.upperLimit != '') {
            return `Last ${newLimits.upperLimit} days other income`
        }
...
export default AddOtherIncomeReport;