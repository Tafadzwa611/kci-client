...
const UpdatePermsForm = ({ user, allPerms }) => {
  ...
  const initialValues = {
    users__toplevelperms: user.perms.users__toplevelperms
      ? user.perms.users__toplevelperms.map(perm => ({ value: perm.id, label: perm.name }))
      : [],
    clients__client: user.perms.clients__client
      ? user.perms.clients__client.map(perm => ({ value: perm.id, label: perm.name }))
      : [],
    clients__group: user.perms.clients__group
      ? user.perms.clients__group.map(perm => ({ value: perm.id, label: perm.name }))
      : [],
    loans__loan: user.perms.loans__loan
      ? user.perms.loans__loan.map(perm => ({ value: perm.id, label: perm.name }))
      : [],
    reports__dataexportreport: user.perms.reports__dataexportreport
      ? user.perms.reports__dataexportreport.map(perm => ({ value: perm.id, label: perm.name }))
      : [],
    expenses__expense: user.perms.expenses__expense
      ? user.perms.expenses__expense.map(perm => ({ value: perm.id, label: perm.name }))
      : [],
    otherincome__otherincome: user.perms.otherincome__otherincome
      ? user.perms.otherincome__otherincome.map(perm => ({ value: perm.id, label: perm.name }))
      : [],
    accounting__journal: user.perms.accounting__journal
      ? user.perms.accounting__journal.map(perm => ({ value: perm.id, label: perm.name }))
      : [],
    ...
    reports__rightssupport: user.perms.reports__rightssupport
      ...
      : [],
    admin_perms: user.perms.admin_perms
      ? user.perms.admin_perms.map(perm => ({ value: perm.id, label: perm.name }))
      : []
  };
            ...
            <NonFieldErrors errors={errors}>
              <div className='sf-shell'>
                <div className='sf-shell-head'>
                  <div className='sf-shell-title'>Permissions</div>
                  <div className='sf-shell-subtitle'>
                    Update the staff member’s module and operational permissions across the available permission groups.
                  </div>
                </div>
                ...
                <div className='sf-shell-body'>
                  <section className='sf-section'>
                    <div className='sf-section-head'>
                      <div className='sf-section-title'>Permission information</div>
                      <div className='sf-section-hint'>
                        Select the permissions to assign for each category, then save to update the user’s access.
                      </div>
                    </div>
