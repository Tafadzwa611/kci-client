      ...
      {(operatingExpenses.length > 0) ? operatingExpenses.map((ie, idx) => {
        return (
          <tr key={idx}>
            ...
            <td align='right'><input type='text' name={ie.name} autoComplete='off' value={`${currencyIso} ${ie.amount}`} readOnly={true} className='custom-select-form row-form input-background'/></td>
          </tr>
        )
      }) : 
      <tr>
