...
function AddJournal() {
  ...
  return (
    <>
      {/* keep logic: still a button + link */}
      <div>
        <button type="button" className="btn btn-default max" style={{marginBottom:'1rem'}}>
                  ...
                  <div className="sf-page">
                    <div className="sf-shell">
                      <div className="sf-shell-head">
                        <div className="sf-shell-title">Add Journal</div>
                        <div className="sf-shell-subtitle">
                          Create a journal entry by selecting currency, branches, accounts, and posting details.
                        </div>
                      </div>
                      ...
                      <div className="sf-shell-body">
                        ...
                        {journalIds.length > 0 ? (
                          <section className="sf-section">
                            ...
                            <div className="sf-section-body" style={{ display: "grid", gap: 8 }}>
                              {journalIds.map((journalId, idx) => (
                                <Notification key={idx} journalId={journalId} />
                              ))}
                            </div>
                          </section>
                        ) : null}
                        ...
                        <section className="sf-section">
                          <div className="sf-section-head">
                            <div className="sf-section-title">Journal Information</div>
                            <div className="sf-section-hint">All fields are stacked vertically.</div>
                          </div>

                          {/* ✅ vertical stack (no flex rows) */}
                          <div className="sf-section-body" style={{ display: "grid", gap: 12 }}>
                            ...
                            <div className="add_journal_field_container">
                              <CustomSelectForm
                            ...
                            {values.branch_debited_id === values.branch_credited_id ? (
                              <CustomCheckbox label="Redirect" name="redirect" />
                            ) : null}
                          </div>
                        </section>
                      </div>
  ...
  if (Object.keys(journalId).length === 1) {
    return (
      <div className="success__submit" style={{ display: "flex", columnGap: "5px" }}>
        ...
        <Link to={`/accounting/viewaccounting/journals/journal/${journalId.id}`}>View Journal</Link>
      </div>
    );
  }
  return (
    <div className="success__submit" style={{ display: "flex", columnGap: "5px" }}>
      <div>Journals created</div>
      <Link to={`/accounting/viewaccounting/journals/journal/${journalId.debit_journal_id}`}>View Debit Journal</Link>
      <Link to={`/accounting/viewaccounting/journals/journal/${journalId.credit_journal_id}`}>View Credit Journal</Link>
    </div>
  );
};
...
const CustomSelectForm = ({ label, ...props }) => {
  ...
  return (
    <div className="custom-background add_journal_field_section">
