...
function AddExpensesReport({open, setOpen, setExpensesR, branches, currencies}) {
    const [limits, setLimits] = useState({upperLimit: ''});
    const [currencyId, setCurrencyId] = useState('');
    const [selectedBranchesIds, setSelectedBranchesIds] = useState([]);
    const [expensesrname, setExpensesRName] = useState('All time expenses');
    ...
    async function fetchExpensesReport() {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setOpen(false);
                setLoading(false);
                return setExpensesR(curr => [...curr, {...data, currencyIso: currencies.filter(currency => currency.id == currencyId)[0].shortname, currencyId, selectedBranchesIds, limits}]);
            }else {
    ...
    const getUrl = () => {
        return `/expensesapi/expenses_report/?currency_id=${currencyId}
            ...
            <div className='modal-dialog modal-lg' style={{maxWidth:"calc(100% - 3rem)"}}>
                <div className='modal-content'>
                    <div className='modal-header'>
                        <span style={{fontWeight:"600"}}>{expensesrname}</span>
                    ...
                    <ModalBody
                        ...
                        setExpensesRName={setExpensesRName}
                    />
...
const ModalBody = ({
    ...
    const handleLimits = (evt) => {
        ...
        setLimits(newLimits);
        const newExpensesName = getExpensesReportName(newLimits);
        setExpensesRName(newExpensesName);

        if (newLimits.upperLimit === '') {return}

        if (Number(newLimits.upperLimit) <= 0) {
        setExpensesRName('Error');
        return setErrorMsg('Upper Limit cannot be zero or than Lower zero.');
        }
    }

    const getExpensesReportName = (newLimits) => {
        if (newLimits.upperLimit == '') {
            return 'All time expenses'
        }

        if (newLimits.upperLimit == 1) {
            return 'From yesterday expenses'
        }
        
        if (newLimits.upperLimit != '') {
            return `Last ${newLimits.upperLimit} days expenses`
        }
...
export default AddExpensesReport;