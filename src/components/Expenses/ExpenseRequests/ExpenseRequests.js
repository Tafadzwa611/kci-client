...
function Filter({ setRequests, setParams }) {
  ...
  const onSubmit = async (values, actions) => {
    try {
      ...
      setParams(params);
      const response = await axios.get('/expensesapi/list_expense_request/', { params: params });
...
function ApproveExpense({ request, setOpen, setRequests }) {
  ...
  const onSubmit = async (values, actions) => {
    try {
      ...
      const response = await axios.post(`/expensesapi/approve_expense_request/${request.id}/`, values, CONFIG);
        ...
        {({ errors, isSubmitting }) => (
          <Form>
            <div className='title' style={{ fontSize: '0.875rem' }}>
              ...
              <b>
                Amount: {request.expense.expense_amount}<br />
                Requested By: {request.requested_by}<br />
                Date Requested: {request.expense.expense_date}<br />
              </b>
            </div>
...
const RejectRequest = ({ setOpen, request, setRequests }) => {
  ...
  const onSubmit = async (values, actions) => {
    try {
      ...
      const response = await axios.post(`/expensesapi/reject_expense_request/${request.id}/`, values, CONFIG);
      const updates = response.data;
      updateItem(
        request.id,
        updates.status,
  ...
  return (
    <Modal open={true} setOpen={setOpen} title={'Reject Expense Request'}>
...
const Pager = ({ prevPageNumber, nextPageNumber, setData, params }) => {
  ...
  const onClick = async (evt) => {
    try {
      ...
      const response = await axios.get('/expenseaspi/list_expense_request/', { params: params });
