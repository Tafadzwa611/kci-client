      ...
      {({ isSubmitting, errors, setFieldValue }) => (
        <Form>
          <NonFieldErrors errors={errors}>
            ...
            >
              <option value=''>------</option>
              {expensetypes.filter(type => type.is_active).map(type => <option key={type.id} value={type.id}>{type.currency__shortname} {type.name} {type.branch__name}</option>)}
            </CustomSelect>
            <CustomSelect label='Fund Account' name='fund_account_id' required>
              <option value=''>------</option>
              {newFundAccountArray.map(account => <option key={account.id} value={account.id}>{account.currency} {account.general_ledger_name} {account.branch}</option>)}
            </CustomSelect>
