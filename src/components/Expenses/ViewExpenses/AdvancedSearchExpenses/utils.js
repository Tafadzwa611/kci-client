const mapRules = rule => {
    const exceptFields = ['branch_id', 'expense_date', 'date_created'];
  ...
  const getAdvOpts = query => {
    return {
      ...
      expense_date: query.rules.filter(rule => rule.field === 'expense_date').map(mapRules),
      expense_name: query.rules.filter(rule => rule.field === 'expense_name').map(mapRules),
      reference: query.rules.filter(rule => rule.field === 'reference').map(mapRules),
      expense_amount: query.rules.filter(rule => rule.field === 'expense_amount').map(mapRules),
