          ...
          <table className='table' style={{width:'100%'}} id='loans-report'>
            ...
            <tbody>
              {report.loans.map((loan, idx) => {
                return (
                  <tr key={idx}>
                    {loan.loan_num ? <td>{loan.branch_name}</td> : <td><b>{loan.branch_name}</b></td>}
                    {loan.loan_num ? <td>{loan.db_date}</td> : <td><b>{loan.db_date}</b></td>}
                    <td>{loan.client_name}</td>
                    <td>{loan.loan_num}</td>
                    <td>{loan.last_date}</td>
                    {loan.loan_num ? <td>{loan.principal}</td> : <td><b>{loan.principal}</b></td>}
                    {loan.loan_num ? <td>{loan.claimable_balance}</td> : <td><b>{loan.claimable_balance}</b></td>}
                    {loan.loan_num ? <td>{loan.balance}</td> : <td><b>{loan.balance}</b></td>}
                    {loan.loan_num ? <td>{loan.current_balance}</td> : <td><b>{loan.current_balance}</b></td>}
                    {loan.loan_num ? <td>{loan.total_penalties}</td> : <td><b>{loan.total_penalties}</b></td>}
                    {loan.loan_num ? <td>{loan.interest_penalty}</td> : <td><b>{loan.interest_penalty}</b></td>}
