                    ...
                    <table className='table' style={{width:'100%'}} id='loans-granted-summary'>
                        ...
                        <tbody>
                            {report.months.map((month, index) => {
                                return (
                                    <tr key={index}>
                                        {month.month_year ? <td>{month.branch}</td> : <td><b>{month.branch}</b></td>}
                                        <td>{month.month_year}</td>
                                        {month.month_year ? <td>{month.principal}</td> : <td><b>{month.principal}</b></td>}
                                        {month.month_year ? <td>{month.interest}</td> : <td><b>{month.interest}</b></td>}
                                        {report.fee_names.map(fee_name => (
                                            month.month_year ? <td key={fee_name}>{month[fee_name]}</td> : <td key={fee_name}><b>{month[fee_name]}</b></td>
                                        ))}
                                        {month.month_year ? <td>{month.claimable_balance}</td> : <td><b>{month.claimable_balance}</b></td>}
                                    </tr>
                                )
                            })}
                        </tbody>
