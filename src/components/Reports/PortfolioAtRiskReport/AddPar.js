...
function AddPar({open, setOpen, setPars, branches, currencies}) {
    const [limits, setLimits] = useState({lowerLimit: '', upperLimit: ''});
    const [currencyId, setCurrencyId] = useState('');
    const [selectedBranchesIds, setSelectedBranchesIds] = useState([]);
    const [parName, setParName] = useState('1+');
    ...
    async function fetchPar() {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setOpen(false);
                setLoading(false);
                return setPars(curr => [...curr, {...data, currencyIso: currencies.filter(currency => currency.id == currencyId)[0].shortname, currencyId, selectedBranchesIds, limits}]);
            }else {
    ...
    const getUrl = () => {
        return `/reportsapi/par-report/?currency_id=${currencyId}
        &branch_ids=${selectedBranchesIds.toString()}${limits.lowerLimit != '' ? `&lower_limit=${limits.lowerLimit}`: ``}
            ...
            <div className='modal-dialog modal-lg' style={{maxWidth:"calc(100% - 3rem)"}}>
                <div className='modal-content'>
                    <div className='modal-header'>
                        <span style={{fontWeight:"600"}}>PAR {parName}</span>
...
const ModalBody = ({
    ...
    const handleLimits = (evt) => {
        ...
        setLimits(newLimits);
        const newParName = getParName(newLimits);
        setParName(newParName);

        if (newLimits.lowerLimit === '' || newLimits.upperLimit === '') {return}

        if (Number(newLimits.upperLimit) - Number(newLimits.lowerLimit) < 0) {
        setParName('Error');
        return setErrorMsg('Upper Limit cannot be less than Lower Limit.');
        }
    }

    const getParName = (newLimits) => {
        if (newLimits.upperLimit == '' && newLimits.lowerLimit == '') {
            return '1+'
        }

        if (newLimits.upperLimit == newLimits.lowerLimit) {
            return `${newLimits.upperLimit}`
        }

        if (newLimits.upperLimit != '' && newLimits.lowerLimit != '') {
            return `${newLimits.lowerLimit} to ${newLimits.upperLimit}`
        }
        ...
        if (newLimits.lowerLimit != '') {
            return `${newLimits.lowerLimit}+`
        }
    }
        ...
        <>
            <div className='modal-body text-light'>
                ...
                <div className='row custom-background' style={{marginTop: '15px'}}>
                    ...
                    <div className='col-9'>
                        <input name='lowerLimit' value={limits.lowerLimit} min='1' type='number' className='custom-select-form' style={{width:"100%"}} onChange={handleLimits}/>
                    </div>
                </div>
